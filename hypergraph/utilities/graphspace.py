"""
.. module:: graphspace
   :synopsis: Provides functionality to post directed hypergraphs to
            GraphSpace (located at www.holmes.cs.vt.edu).  Users must
...
def post_to_graphspace(hypergraph, graphid, username, password, group=None):
    """Posts a directed hypergraph to GraphSpace with a specified graphid from
    a user with username and password.  Optionally shares the hypergraph with
    a group.  This function assumes that nodes are strings.
    ...
    graph_space_graph = Graph(layout='ForceDirected',
                              tags=[],
                              description='')
    ...
    for hyperedge_id in hypergraph.get_hyperedge_id_set():
        ...
        hyperedge_tail_set = \
            hypergraph.get_hyperedge_attribute(hyperedge_id, 'tail')
        hyperedge_head_set = \
            hypergraph.get_hyperedge_attribute(hyperedge_id, 'head')
        hyperedge_weight = \
            hypergraph.get_hyperedge_attribute(hyperedge_id, 'weight')
        ...
        if len(hyperedge_tail_set) == 1 and len(hyperedge_head_set) == 1:
            ...
            label = '%s: weight=%f' % (hyperedge_id, hyperedge_weight)
            graph_space_edge = Edge(id=hyperedge_id,
                                    label=label,
                                    source=hyperedge_tail,
                                    target=hyperedge_head,
                                    color=hyperedge_color,
                                    directed=True,
                                    width=hyperedge_width,
            ...
            square_node = Node(id=hyperedge_id,
                               label=hyperedge_id,
                               color='white',
                               shape='square',
                               size='auto',
    ...
    client = GraphSpace(user=username,
                        password=password,
                        url='http://holmes.cs.vt.edu/')
