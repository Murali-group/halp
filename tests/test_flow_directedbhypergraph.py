...
def test_flow_directedbhypergraph():
    ...
    e1 = DirectedBHyperEdge(set([x2]),set([x1]),1)
    e2 = DirectedBHyperEdge(set([x1]),set([x3]),4)
    e3 = DirectedBHyperEdge(set([x1]),set([x2,x4]),7)
    e4 = DirectedBHyperEdge(set([x3]),set([x4]),5)
